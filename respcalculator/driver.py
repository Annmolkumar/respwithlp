...
def resp(molecules, options_list=None, intermol_constraints=None):
    ...
    if options['RESTRAINT']:
        ...
        if 'MAX_IT' not in options:
            options['MAX_IT'] = 25
    if "LPCOOR" in options:
        lpsym=[];lpcoor=[]
        lpcoorln = options["LPCOOR"].split("\n")
        for line in lpcoorln:
            lpsym.append(line.split()[0])
    ...
    for imol in range(len(molecules)):
        ...
        coordinates = np.concatenate((coordinates,lpcoor),axis=0)
        options['coordinates'] = coordinates
        symbols = np.concatenate((symbols,lpsym),axis=0)
        options['symbols'] = symbols
        symbols_list[-1] = symbols
